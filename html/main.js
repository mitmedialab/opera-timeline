(function () {
  ...
  class TimelineEvent {
    ...
    constructor(d) {
      ...
      if (typeof d.date === 'string') {
        ...
        else if (date = parseYear$1(d.date)) d.datePrecision = 'year';
        if (date) d.date = date;
      }
  ...
  const formatMonthDay   = timeFormat('%b %e');
  const formatMonthLong  = timeFormat('%B'); // %B=January %b=Jan
  ...
  const parseYear$2 = timeParse('%Y');
  const dateToYearFloat = scaleTime()
    .domain([parseYear$2(1000), parseYear$2(2000)])
    .range([1000, 2000]);

  const yearFloatToDate = dateToYearFloat.invert;
  ...
  class Timeline {
    ...
    constructor(parent, options = {}) {
      ...
      this.eventAxis = axisRight(this.yScale)
        .tickSizeOuter(0);
      this.eventAxisGroup = this.gOuter.append('g')
        .attr('class', 'timeline-event-axis')
        .call(this.eventAxis);
      
      this.mouseEmitter = new EventEmitter();
      /**
  ...
  class MasterTimeline {
    ...
    constructor(parent, drawDebugAxes) {
      ...
      this.content = {};
      this.lastZoomTransform = identity$1;
      ...
      this.zoom = zoom()
        ...
        .wheelDelta(() => {
          ...
          return -event.deltaY * (event.deltaMode === 1 ? 0.05 : event.deltaMode ? 1 : pixelRatio);
        })
        .scaleExtent([0.4, 60])
        .translateExtent([
          [0, this.yScale(1400)],
          [0, this.yScale(2300)]])
        .on('start.emit', () => { this.emitter.emit('zoom', event); })
        .on('zoom.emit', () => { this.emitter.emit('zoom', event); })
        .on('end.emit', () => { this.emitter.emit('zoom', event); })
  ...
  const formatSmart = function(date) { 
    ...
    if (date.precision === 'year') return formatYear$1(date);
    if (date.precision === 'month') return formatMonth(date);
  };

  const styles = [
    { ticks: year.every(10), eventFormat: formatYear$1 },
    { ticks: year.every(1),  eventFormat: formatSmart },
  ];
  ...
  var data = { "The Brain Opera":{ date:1996,
      video:{ url:"opera/1996-brain-opera-at-lincoln-center.mp4",
        startTime:3 },
      detail:"The Brain Opera premiered at Lincoln Center in 1996.\n",
      html:"<p>The Brain Opera premiered at Lincoln Center in 1996.</p>\n" },
    "Death and the Powers Live":{ date:"2014-2",
      image:{ url:"opera/powers-live.jpg" },
      detail:"Adding a another layer on top of an already complex show...\n",
      html:"<img src=\"opera/powers-live.jpg\"/><p>Adding a another layer on top of an already complex show...</p>\n" } };

  const operaContent = createTimelineEvents(data);

  // Setup the MasterTimeline
  const parentSelection = select('div#media-timeline');
